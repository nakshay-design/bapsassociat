...
// ─── Defaults — NO static image URLs, all come from ACF ──────────────────────
...
export default function About() {
  ...
  useEffect(() => {
    const fetchAcf = async () => {
      try {
        ...
        const cardList: any[] = acf.about_commitment_cards || [];
        ...
        const [
          ...
          resolveMediaId(acf.about_banner_image),
          resolveMediaId(acf.about_content_image_1),
          resolveMediaId(acf.about_content_image_2),
          ...cardList.map((card: any) => resolveMediaId(card.card_icon)),
        ]);

        // ── Build resolved commitment cards ───────────────────────────────
        const fallbackIcons = [Users, Target, TrendingUp, Shield];
        const resolvedCards: CommitmentCard[] = cardList.map((card: any, i: number) => ({
          card_icon: card.card_icon ?? "",
          card_icon_url: cardIconUrls[i] ?? "",
          card_title: card.card_title?.trim() ?? "",
          card_description: card.card_description?.trim() ?? "",
          fallback_icon: fallbackIcons[i] ?? Users,
        }));
        ...
        setAboutData((prev) => ({
          ...
          // Banner
          about_banner_heading: acf.about_banner_heading?.trim() || prev.about_banner_heading,
          about_banner_description: acf.about_banner_description?.trim() || prev.about_banner_description,
          ...
          // Content
          about_content_heading: acf.about_content_heading?.trim() || prev.about_content_heading,
          about_content_description_1: acf.about_content_description_1?.trim() || prev.about_content_description_1,
          about_content_description_2: acf.about_content_description_2?.trim() || prev.about_content_description_2,
          ...
          // Commitment
          about_commitment_heading: acf.about_commitment_heading?.trim() || prev.about_commitment_heading,
          about_commitment_description: acf.about_commitment_description?.trim() || prev.about_commitment_description,
          about_commitment_cards: resolvedCards.length ? resolvedCards : prev.about_commitment_cards,

          // Vision & Mission
          about_vision_title: acf.about_vision_title?.trim() || prev.about_vision_title,
          about_vision_text: acf.about_vision_text?.trim() || prev.about_vision_text,
          about_mission_title: acf.about_mission_title?.trim() || prev.about_mission_title,
          about_mission_text: acf.about_mission_text?.trim() || prev.about_mission_text,
        }));
      } catch (err) {
  ...
  return (
    <div className="w-full overflow-hidden">
      ...
      <section className="relative py-24 bg-primary text-white">
        <div className="absolute inset-0 overflow-hidden">
          {about_banner_image_url ? (
            ...
            /* subtle animated gradient fallback when no banner image */
            <div className="absolute inset-0 bg-gradient-to-br from-primary via-primary/90 to-primary/70 animate-[pulse_8s_ease-in-out_infinite]" />
          )}
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <FadeIn direction="right">
              <h2
                className="text-3xl md:text-5xl font-display font-bold text-heading mb-6"
                dangerouslySetInnerHTML={{ __html: about_content_heading }}
              />
            ...
            {/* Images — fully dynamic from ACF */}
            <FadeIn direction="left" className="grid grid-cols-2 gap-4">
              {about_content_image_1_url ? (
                <img
                  src={about_content_image_1_url}
                  alt="Team"
                  className="rounded-3xl shadow-lg w-full h-64 object-cover"
                />
              ) : (
                <div className="rounded-3xl bg-secondary/30 w-full h-64 flex items-center justify-center">
                  <span className="text-muted-foreground text-sm">{loading ? "Loading…" : "No image"}</span>
                </div>
              )}
              {about_content_image_2_url ? (
                <img
                  src={about_content_image_2_url}
                  alt="Partnership"
                  className="rounded-3xl shadow-lg w-full h-64 object-cover mt-8"
                />
              ) : (
                <div className="rounded-3xl bg-secondary/30 w-full h-64 flex items-center justify-center mt-8">
                  <span className="text-muted-foreground text-sm">{loading ? "Loading…" : "No image"}</span>
                </div>
              )}
            </FadeIn>
      ...
      {/* ── Commitment / Values & Mission Section ─────────────────────────── */}
      <section className="py-24 bg-secondary/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Commitment Cards — icon from ACF, Lucide fallback */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-20">
            {about_commitment_cards.map((card, i) => {
              ...
              return (
                <FadeIn key={i} delay={i * 0.1}>
                  <div className="bg-white p-8 rounded-3xl shadow-lg border border-border h-full text-center hover:-translate-y-2 transition-transform duration-300">
                    ...
                    <h3 className="text-xl font-bold text-heading mb-3">{card.card_title}</h3>
                    <p className="text-foreground">{card.card_description}</p>
                  </div>
                </FadeIn>
              );
            })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <FadeIn delay={0.2}>
              <div className="bg-primary text-white p-10 rounded-3xl shadow-xl h-full">
                ...
                <p className="text-lg leading-relaxed text-white/90">{about_vision_text}</p>
              </div>
            </FadeIn>
            <FadeIn delay={0.4}>
              <div className="bg-white border-2 border-primary p-10 rounded-3xl shadow-xl h-full">
                ...
                <p className="text-lg leading-relaxed text-foreground">{about_mission_text}</p>
              </div>
            </FadeIn>
          </div>
